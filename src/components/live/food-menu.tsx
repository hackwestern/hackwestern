...
const FoodMenu = () => {
  ...
  ].filter(Boolean).filter((v): v is string => v !== null);

  const days = Object.keys(allMeals) as (keyof typeof allMeals)[];

  return (
    <div className="overflow-auto h-screen pb-48">
    <Accordion
      type="multiple"
      className="w-[50%]"
    ...
    >
      ...
      <AccordionBorderlessItem key={day} value={day}>
        <AccordionTrigger>
          <h1 className="font-jetbrains-mono text-heavy text-lg md:text-xl">{day.toUpperCase()}</h1>
        </AccordionTrigger>
            ...
            <div className="flex flex-col gap-4 w-full">
              {(() => {
                ...
                return mealKeys.map((meal) => {
                  ...
                  return (
                    <div key={meal} className="flex flex-col gap-4 w-full">
                      <h2 className="font-jetbrains-mono text-medium">
                        {meal.toUpperCase()}
                      </h2>
                      {options.map((option: MealData) => (
                        <div key={option.name} className="rounded-xl border border-[#f1f0f2] bg-white p-4 flex flex-row justify-between w-full">
                          <div className="flex flex-col justify-center">
                            <h2 className="font-figtree text-heavy font-medium">
                              {option.name}
                            </h2>
                            <h3 className="font-figtree font-italic text-medium">
                              {option.vendor}
                            </h3>
                          </div>
                          <Popover
                          ...
                          >
                            <PopoverTrigger asChild>
                              <button
                                type="button"
                                className={`px-2 py-1 font-figtree text-sm font-medium rounded-md flex flex-row gap-2 items-center ${
                            ...
                            <PopoverContent side="right" align="center" className="w-fit">
                              <Table>
                                <TableHeader>
                                  <TableRow className="font-figtree text-heavy">
                                    {(Object.keys(option.allergens) as (keyof typeof option.allergens)[]).map((allergen) => (
                                      <TableHead key={String(allergen)}>{String(allergen)}</TableHead>
                                    ))}
                                  </TableRow>
                                </TableHeader>
                                <TableBody>
                                  <TableRow>
                                    {Object.entries(option.allergens ?? {}).map(([allergen, value]) => {
                                      const v = value as boolean | "May Contain" | null | undefined;
                                      let cell: React.ReactNode;
                                      if (v === true) cell = "✅";
                                      else if (v === false) cell = "❌";
                                      else if (v === "May Contain") cell = "May Contain";
                                      else cell = String(v ?? "");
                                      return <TableHead key={String(allergen)}>{cell}</TableHead>;
                                    })}
                                  </TableRow>
                                </TableBody>
                              </Table>
