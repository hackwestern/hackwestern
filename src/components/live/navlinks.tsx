...
export const SectionLink = ({ tab, name }: { tab: string; name: string }) => {
  ...
  return (
    <Link
      href={`live/?tab=${tab}`}
      className={`flex items-center py-3 gap-3 px-4 py-1.5 font-figtree ${isActive ? "bg-primary-300 text-heavy" : "text-medium"} rounded-md transition-all hover:bg-primary-300`}
    >
...
export const LogisticsLink = ({ step, name }: { step: string; name: string }) => {
  const searchParams = useSearchParams();
  ...
  return (
    <Link
      href={`live/?tab=event-logistics&step=${step}`}
      className={`flex items-center gap-3 w-full px-4 py-2.5 font-figtree ${isActive ? "bg-primary-300 text-heavy" : "text-medium"} rounded-md transition-all hover:bg-primary-300`}
    >
