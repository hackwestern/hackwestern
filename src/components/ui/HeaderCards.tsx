...
function HeaderCards() {
	const names = projectsPlaceHolder[0].map(([image, description, url]) => ({
		image,
		description,
		url,
	}));
			...
			{names.map((name, i) => {
				...
				return (
					<motion.div
						...
						className="group pointer-events-auto absolute relative flex h-[292px] w-[369px] flex-col items-center rounded-2xl border border-gray-200 bg-white p-[11px] shadow-xl cursor-pointer transition-transform hover:scale-105"
					>
						<motion.div
