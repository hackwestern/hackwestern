...
export default async function handler(
  ...
  if (!GOOGLE_WALLET_ISSUER_ID) {
    ...
    return res.status(500).json({ error: "Server configuration error: Missing Wallet Issuer ID." });
  }
  if (!GOOGLE_APPLICATION_CREDENTIALS_PATH) {
    console.error("FATAL: GOOGLE_APPLICATION_CREDENTIALS environment variable (path to key file) is not set.");
    return res.status(500).json({ error: "Server configuration error: Missing credentials path." });
  }
  if (!WALLET_SERVICE_ACCOUNT_EMAIL) {
    console.error("FATAL: WALLET_SERVICE_ACCOUNT_EMAIL environment variable is not set.");
    return res.status(500).json({ error: "Server configuration error: Missing service account email." });
  }
  if (!NEXT_PUBLIC_APP_URL) {
    console.error("FATAL: NEXT_PUBLIC_APP_URL environment variable is not set (needed for JWT origins).");
    return res.status(500).json({ error: "Server configuration error: Missing public app URL." });
  }
  ...
  if (!serviceAccountCreds) {
    ...
    console.error("FATAL: Service account credentials could not be loaded or parsed.");
    return res.status(500).json({ error: 'Service account credentials not loaded.' });
  }

  const session = await getServerSession(req, res, authOptions);
  if (!session?.user?.id || !session?.user?.email) {
    return res.status(401).json({ error: 'Unauthorized: User not logged in or email missing.' });
  }
  ...
  try {
    ...
    let passClass;
    try {
      console.log(`Attempting to get class with classId (which is the resourceId): ${classId}`);
      // VVVVV CHANGE THIS LINE VVVVV
      const getClassResponse = await walletClient.genericclass.get({ resourceId: classId });
      // ^^^^^ CHANGE THIS LINE ^^^^^
      passClass = getClassResponse; 
      console.log("Successfully fetched class:", passClass.data.id);
    } catch (err: any) {
      if (err.code === 404) {
        console.log(`Class ${classId} not found, attempting to create new class.`);
        ...
        const newClassPayload: google.walletobjects_v1.Schema$GenericClass = {
              ...
              cardRowTemplateInfos: [
                {
                  twoItems: {
                    startItem: {
                      firstValue: {
                        defaultValue: { language: "en", value: "Attendee Name" },
                      },
                    },
                    endItem: {
                      firstValue: {
          ...
          logo: {
            ...
            contentDescription: { defaultValue: { language: "en", value: "HackWestern Class Logo" } },
          },
          hexBackgroundColor: "#4285f4",
        };
        console.log("New class payload:", JSON.stringify(newClassPayload, null, 2));
        try {
          const classResponse = await walletClient.genericclass.insert({ requestBody: newClassPayload });
          ...
          if (insertErr.response?.data?.error) {
            console.error("Detailed error inserting new class:", JSON.stringify(insertErr.response.data.error, null, 2));
          }
          throw insertErr; 
        }
      } else {
        ...
        if (err.response?.data?.error) {
            console.error("Detailed error fetching class:", JSON.stringify(err.response.data.error, null, 2));
        }
        throw err; 
      }
    }

    if (!passClass?.data) { // Check if passClass or passClass.data is still undefined/null
        console.error("FATAL: Class data is missing after get/create attempt.");
        return res.status(500).json({ error: "Server error: Failed to obtain class definition." });
    }
    ...
    const newObject: google.walletobjects_v1.Schema$GenericObject = {
      ...
      logo: {
        sourceUri: { uri: "https://picsum.photos/200" }, // Generic PNG/JPG
        contentDescription: { defaultValue: { language: "en", value: "HackWestern Object Logo" } },
      },
      cardTitle: {
      ...
      heroImage: {
        ...
        contentDescription: { defaultValue: { language: "en", value: "HackWestern Event Banner"} }
      },
      textModulesData: [
        {
    ...
    console.log("Attempting to insert object with payload:", JSON.stringify(newObject, null, 2));

    const objectResponse = await walletClient.genericobject.insert({ requestBody: newObject });
    ...
    const token = jwt.sign(claims, serviceAccountCreds.private_key, { algorithm: 'RS256' });
    ...
    if (error.response?.data?.error) {
      console.error('Detailed Google Wallet API error (outer catch):', JSON.stringify(error.response.data.error, null, 2));
      return res.status(500).json({ error: `Google Wallet API Error: ${error.response.data.error.message}` });
    } else if (error.isAxiosError && error.response) {
        console.error('Axios error response data (outer catch):', JSON.stringify(error.response.data, null, 2));
    }
    return res.status(500).json({ error: 'Failed to create wallet pass.', message: String(error.message) });
  }
}
