...
vi.mock("googleapis", () => ({
  google: {
    ...
    auth: {
      // Also mock auth if it's directly used, though JWT from google-auth-library is more common
...
describe("/api/wallet/create-pass API Route", () => {
  ...
  beforeEach(() => {
    ...
    process.env.WALLET_SERVICE_ACCOUNT_EMAIL =
    ...
    (vi.mocked(require("fs").readFileSync) as any).mockReturnValue(
      dummyKeyContent,
    );
  });
  ...
  describe("Successful Pass Creation", () => {
    it("should return 200 and saveToWalletUrl on successful pass creation (new object)", async () => {
      ...
      mockGenericObjectInsert.mockResolvedValue({
        data: {
          id: "test-issuer-id.object-123",
          classId: "test-issuer-id.HackWesternUserPass",
        },
      });
    ...
    it("should return 200 and saveToWalletUrl if object already exists (409 then get)", async () => {
      ...
      // @ts-ignore
      mockDbSelect.limit.mockResolvedValue([MOCK_DB_USER]);
      mockGenericObjectInsert.mockRejectedValue({
        code: 409,
        message: "Object already exists.",
      }); // Simulate 409
      mockGenericObjectGet.mockResolvedValue({
  ...
  describe("Google Wallet API Errors", () => {
    ...
    it("should return 500 if genericobject.insert fails (409) and subsequent .get also fails", async () => {
      ...
      mockGenericObjectGet.mockRejectedValue({
        code: 500,
