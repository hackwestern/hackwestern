...
/** Whether this user already has a stored result for every hacker check type. */
export async function hasAllHackerChecks(userId: string): Promise<boolean> {
  const rows = await db
    .select({ checkType: hackerCheckResults.checkType })
    .from(hackerCheckResults)
    .where(eq(hackerCheckResults.userId, userId));
