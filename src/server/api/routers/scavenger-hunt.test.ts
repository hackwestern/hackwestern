...
describe("scavenger.scan", async () => {
	beforeEach(async () => {
		await db.delete(scavengerHuntScans)
			.where(inArray(scavengerHuntScans.itemId, [testItem?.id].filter((v): v is number => !!v)));

		await db.delete(scavengerHuntItems)
			.where(inArray(scavengerHuntItems.code, ["TESTCODE123", "TESTCODE456"]));

		const [item] = await db.insert(scavengerHuntItems)
			.values({
				code: "TESTCODE123",
	...
	test("allows scanning multiple distinct items", async () => {
		const [item2] = await db.insert(scavengerHuntItems)
			.values({
				code: "TESTCODE456",
		...
		await caller.scavenger.scan({ userId: session.user.id, itemCode: testItem.code });
