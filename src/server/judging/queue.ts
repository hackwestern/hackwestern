 ...
 */
export async function pickNextTeam(
  judgeId: string,
  isSponsored: boolean,
...
export async function deleteMark(teamMarkId: number): Promise<void> {
 ...
 */
export async function skipCurrent(judgeId: string): Promise<string> {
  return db.transaction(async (tx) => {
    const held = await tx.query.judgingQueue.findFirst({
      where: eq(judgingQueue.currentJudgeId, judgeId),
    });
 ...
 */
export async function forceAssign(
  judgeId: string,
  teamId: string,
): Promise<void> {
  return db.transaction(async (tx) => {
    ...
    await tx
      .update(judgingQueue)
      .set({ currentJudgeId: judgeId, status: "assigned", assignedAt: new Date() })
      .where(eq(judgingQueue.teamId, teamId));
  });
}
 ...
 */
export async function loadQueue(roundsPerTeam: number): Promise<number> {
  return db.transaction(async (tx) => {
...
export async function getSubmittedMarks(judgeId: string) {
